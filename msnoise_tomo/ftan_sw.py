        ...
        for job in jobs:
            ...
            for filter in filters:
                ...
                for i, filename in enumerate(SACfilelist):
                    ...
                    np.savetxt('write_amp.txt',filtrate(np.loadtxt("write_amp.txt")))
...
def filtrate(amp):
    ampmin=np.percentile(amp, 0.001)
    ampmax=np.percentile(amp, 99.99)
    amp = np.clip(amp, a_min=ampmin, a_max=ampmax)
    amp=(amp-ampmin)/(ampmax-ampmin)
    return amp

def save_graphs():
    DIR = os.path.join(os.getcwd(),"RAW_FTAN_FILES/")
    directory=DIR
    db = connect()
    filters = get_filters(db)

    def plot_save(directory,wave):

        for file in os.listdir(directory):
            path = os.path.join(directory, file, wave)
    ...
    for filter in filters:
        directory = os.path.join(DIR, "%02i" % filter.ref)
        plot_save(directory,"RW")
        plot_save(directory,"LW")